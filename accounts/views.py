...
class UserFollowView(LoginRequiredMixin, View):
    def get(self, request, user_id):
        ...
        relation = Relation.objects.filter(from_user=request.user, to_user=user)
        if relation.exists():
            messages.error(request, 'user already followed', 'danger',)
        else:
...
class UserUnfollowView(LoginRequiredMixin, View):
    def get(self, request, user_id):
        ...
        if relation.exists():
            ...
            messages.error(request, 'user not followed this user', 'danger')
        return redirect('accounts:user_profile', user.id)
...
class UserEditProfileView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            request.user.save()
            messages.success(request, 'user_profile edited successfully', 'success')
